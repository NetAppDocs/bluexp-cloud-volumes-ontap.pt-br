...
=== Requisitos para categorizar dados para um locatário do Azure

Você pode usar o mesmo conetor para categorizar dados para vários locatários do Azure. Para habilitar a disposição em camadas para um locatário do Azure em um cenário em que o ambiente de trabalho do Cloud Volumes ONTAP é criado por um locatário e o conetor é criado por outro, verifique o seguinte:

* Você inseriu as credenciais para ambos os locatários no BlueXP  para validação cruzada. Consulte a https://docs.netapp.com/us-en/bluexp-setup-admin/task-adding-azure-accounts.html["Gerencie as credenciais do Azure e as assinaturas do marketplace para o BlueXP"^].
* Você ativou o peering de rede virtual (VNet) entre os dois https://learn.microsoft.com/en-us/azure/virtual-network/virtual-network-peering-overview["No portal do Azure"^] locatários . Isso é necessário para que o conetor possa acessar a conta de armazenamento do outro locatário.
* Você tem link:task-enabling-private-link.html#disable-azure-private-links-and-use-service-endpoints-instead["Links privados do Azure desativados"] que usar o endpoint do serviço de sub-rede.
